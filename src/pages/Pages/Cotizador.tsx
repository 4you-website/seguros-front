...
const Cotizador = () => {
    ...
    const buscarPorCodigoPostal = async () => {
        ...
        try {
            // ✅ Llamamos a la API externa, como al principio
            ...
            // 🔍 Matcheamos el nombre de provincia de la API externa
            // con el nombre de tu API /states para obtener el ID
            const stateMatch = findStateByProvinciaNombre(provinciaApiNombre, states);

            if (!stateMatch) {
                // No encontramos la provincia en /states: ayudamos pero dejamos elegir manualmente
                Swal.fire({
                    icon: "info",
                ...
                setForm((prev) => ({
                    ...
                    provincia: "", // sin ID: usuario deberá elegir
                }));

                // Para localidades.json usamos el nombre de la provincia
                setSelectedProvinciaNombre(provinciaApiNombre);
                setLocalidades(getLocalidadesByProvincia(provinciaApiNombre));
                return;
            }

            // ✅ Caso OK: tenemos CP, localidad, nombre de provincia y el ID de tu API
            setForm((prev) => ({
                ...prev,
                codpostal: codigo,
                localidad: localidadApi,
                provincia: stateMatch.id.toString(), // 👈 ESTE ID es el que se manda a /assurance/cotizar
            }));

            // Para el combo de localidades seguimos usando el nombre
            setSelectedProvinciaNombre(provinciaApiNombre);
            setLocalidades(getLocalidadesByProvincia(provinciaApiNombre));
        } catch (error) {
            console.error(error);
    ...
    const cotizar = async () => {
        try {
            ...
            Swal.fire({
                icon: "success",
                title: "Cotización obtenida desde la API",
                toast: true,
                position: "top",
                showConfirmButton: false,
                timer: 3000,
                ...
                <form className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            options={states.map((s) => ({
                                value: s.id.toString(), // 👉 ID de la API
                                label: s.name,          // Nombre visible (API)
                            }))}
                            value={
                                form.provincia
                                    ? {
                                        ...
                                        label:
                                            states.find((s) => s.id.toString() === form.provincia)
                                                ?.name || "",
                                    }
                                    : null
                            }
                            onChange={(selected) => {
                                ...
                                // Guardamos el ID de la provincia para mandarlo a /assurance/cotizar
                                setForm((prev) => ({
                                    ...prev,
                                ...
                                // 👇 Intentamos mapear el nombre de la API al nombre de localidades.json
                                ...
                                if (nombreLocalidades) {
                                    ...
                                    // Si no encontramos match, limpiamos localidades y dejamos que el usuario las escriba de otra forma
                                    setSelectedProvinciaNombre("");
                                    setLocalidades([]);
                                    setForm((prev) => ({ ...prev, localidad: "" }));
                                }
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            value={
                                form.localidad
                                    ? { value: form.localidad, label: form.localidad }
                                    : null
                            }
                            onChange={(selected) =>
                                setForm((prev) => ({
                    ...
                    <div>
                        ...
                        <Select
                            id="modelo"
                            placeholder={
                                loadingModels ? "Cargando modelos..." : "Seleccione un modelo"
                            }
                            ...
                            {COMPANIAS.map((c) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-4 py-1.5 rounded-full text-sm font-medium border transition ${isActive
                                            ? "bg-primary text-white border-primary shadow-sm"
                                            : "bg-white dark:bg-[#1b2e4b] text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 hover:bg-primary/10"
                                            }`}
                                    >
                                        {c.label}
                                    </button>
                        ...
                        <div className="sticky top-[70px] z-20 w-full bg-white dark:bg-[#0d1727] border-b border-gray-200 dark:border-gray-700 py-4 mb-8">
                            ...
                            <div className="mt-4 flex flex-col items-center gap-2">
                                ...
                                <div className="flex flex-wrap justify-center gap-2">
                                    ...
                                    {/* Botones por compañía */}
                                    {COMPANIAS.map((c) => (
                                        <button
                                            key={c.id}
                                            type="button"
                                            onClick={() => setFiltroCompania(c.id as typeof filtroCompania)}
                                            className={`px-3 py-1.5 rounded-full text-xs sm:text-sm border font-medium transition ${filtroCompania === c.id
                                                ? "bg-primary text-white border-primary"
                                                : "bg-white dark:bg-[#1b2e4b] text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-700 hover:bg-primary/10"
                                                }`}
                                        >
                            ...
                            {planesFiltrados.map((plan, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Header */}
                                        <div
                                            className={`bg-gradient-to-r ${color} text-white text-center py-6`}
                                        >
                                            <div className="flex justify-between items-center px-4 text-xs opacity-80">
                                                <span>
                                                    #{plan.idCotizacion}
                                                </span>
                                                <span className="px-2 py-0.5 rounded-full bg-black/30 text-[11px] uppercase tracking-wide">
                                            ...
                                            <p className="text-xs mt-1 opacity-90 px-4">
                                                {plan.cubre}
                                            </p>
                                        ...
                                        <div className="p-6 text-center flex-1">
                                            <ul className="text-gray-700 dark:text-gray-300 text-sm space-y-2 mb-5">
                                                ...
                                                <li>
                                                    ...
                                                    <span className="font-semibold">
                                                        ${formatNumber(plan.sumaAsegurada)}
                                                    </span>
                                                </li>
                                                {plan.ajuste && (
                                                    <li>
